...
opt = lapp[[
   ...
   --presc                    (default 1)            use prescription or nah
]]
...
cdata = provider.trainData.data
clabel = provider.trainData.labels
...
function train()
  ...
  local indices = torch.randperm(cdata:size(1)):long():split(opt.batchSize)
  ...
  for t,v in ipairs(indices) do
    ...
    local inputs = cdata:index(1,v)
    targets:copy(clabel:index(1,v))
