...
if not opt then
   ...
   cmd:text()
   cmd:text('Options:')
   cmd:option('-model', 'linear', 'type of model to construct: linear | mlp | convnet |mixed')
...
elseif opt.model == 'convnet' then
   ...
   if opt.transferF == 'sigm' then
      if opt.pooling =='l2' then
         model = nn.Sequential()
